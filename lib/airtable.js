...
export const TABLES = {
  ...
  transactions: "tblesTCFJcjvosmKC",
  models: "tblIAhlUJOGBiq2nU",
};
...
export const INVENTORY_FIELDS = {
  ...
  detailsSheetPrinted: "Details Sheet Printed",
  currentLocation: "Current Location",
};

export const BRAND_FIELD_ID = "fldH71Wvitkz0c3ZE";
...
export const CURRENT_LOCATION_CHOICES = ["Warehouse", "Daryl's", "On Route", "At Auction"];
...
export const WORK_ORDER_FIELDS = {
  ...
  lastUpdatedBy: "Last Updated By",
  billToConsignor: "Bill to Consignor",
  billAmount: "Bill Amount",
  billedConfirmed: "Billed Confirmed",
};
...
export const PART_CATEGORY_CHOICES = [
  "Gun",
  "Wheel",
  "Bill Acceptor",
  "Pwr Supply",
...
export const TRANSACTION_TYPE_CHOICES = [
  "Received", "Checked Out", "Returned", "Used", "Moved",
  "Adjusted", "Damaged", "Lost", "Reserved", "Transferred", "Count Adjustment",
];
...
export async function createRecordsBatch(tableId, recordsFields) {
  // recordsFields: [{ fields }], max 10 per Airtable's API limit
  const chunks = [];
...
export async function logTransaction({ type, partId, inventoryItemId, quantityChange, workOrderId, staffId, notes }) {
  const fields = { [TRANSACTION_FIELDS.type]: type };
  if (partId) fields[TRANSACTION_FIELDS.part] = [partId];
  if (inventoryItemId) fields[TRANSACTION_FIELDS.inventoryItem] = [inventoryItemId];
  if (quantityChange != null) fields[TRANSACTION_FIELDS.quantityChange] = quantityChange;
  if (workOrderId) fields[TRANSACTION_FIELDS.workOrder] = [workOrderId];
  if (staffId) fields[TRANSACTION_FIELDS.staff] = [staffId];
  if (notes) fields[TRANSACTION_FIELDS.notes] = notes;
...
export async function getSelectFieldChoices(tableId, fieldId) {
