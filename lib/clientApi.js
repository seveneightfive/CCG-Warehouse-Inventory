...
export async function patchRecord(kind, id, fields) {
  ...
  try {
    ...
    if (!navigator.onLine) {
      await db.outbox.add({ kind: "patch", table: kind, recordId: id, fields, createdAt: Date.now() });
      await db[kind === "inventory" ? "inventory" : "workOrders"].update(id, fields); // optimistic local update
      return { queued: true };
    }
    throw err;
  }
